...
def extract_translatable_html(input_path, lang_code):
    ...
    def traverse_and_extract(element, block_counter):
        if isinstance(element, NavigableString) and is_translatable_text(element):
            ...
            if text:
                ...
                if sentence_tokens:
                    block_id = f"BLOCK_{block_counter}"
                    tag_name = element.parent.name if element.parent else "unknown"
                    structured_output[block_id] = {"tag": tag_name, "tokens": structured}
                    flattened_output.update(flattened)
        ...
        if hasattr(element, "attrs"):
            for attr in TRANSLATABLE_ATTRS:
                if (
                    attr in element.attrs and isinstance(element[attr], str) and
                    ...
                    if value:
                        ...
                        flattened_output.update(flattened)
                        if sentence_tokens:
                            element[attr] = sentence_tokens[0][0]
                        block_counter += 1
        ...
        if element.name == "meta":
            ...
            if content and (
                (name in SEO_META_FIELDS["name"]) or
                (prop in SEO_META_FIELDS["property"])
            ):
                block_id = f"BLOCK_{block_counter}"
                ...
                flattened_output.update(flattened)
                if sentence_tokens:
                    element["content"] = sentence_tokens[0][0]
                block_counter += 1

        if element.name == "title" and element.string:
            ...
            if text:
                ...
                flattened_output.update(flattened)
                if sentence_tokens:
                    element.string.replace_with(sentence_tokens[0][0])
                block_counter += 1

        if element.name == "script" and element.get("type") == "application/ld+json":
            try:
                raw_json = element.string.strip()
                data = json.loads(raw_json)
                block_counter = extract_from_jsonld(data, block_counter, nlp, structured_output, flattened_output)
                element.string.replace_with(json.dumps(data, ensure_ascii=False, indent=2))
            except Exception as e:
                print(f"⚠️ Failed to parse or process JSON-LD: {e}")

        for child in getattr(element, "contents", []):
            block_counter = traverse_and_extract(child, block_counter)

        return block_counter

    block_counter = traverse_and_extract(soup, block_counter)
