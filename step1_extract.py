...
def extract_translatable_html(input_path, lang_code):
    ...
    position_mapping = {}
    for block_id, block_data in structured_output.items():
        block_type = block_data.get("tag") or block_data.get("attr") or block_data.get("meta") or block_data.get("jsonld")
        if block_type:
            for elem in soup.find_all(True):
                if block_type in elem.attrs.values() or elem.name == block_type:
                    position = get_element_position(elem)
