...
def extract_from_jsonld(obj, block_counter, nlp, structured_output, flattened_output, position_mapping=None, jsonld_pos=1000000):
...
def extract_translatable_html(input_path, lang_code):
    ...
    # Update flattened_output with new block IDs
    ...
    # Update attributes with new block IDs
    for tag in soup.find_all():
        for attr in tag.attrs:
            if isinstance(tag[attr], str) and tag[attr].startswith("BLOCK_"):
                for old_id, new_id in new_block_mapping.items():
                    old_block_num = old_id.split('_')[1]
                    if tag[attr].startswith(f"BLOCK_{old_block_num}"):
                        tag[attr] = tag[attr].replace(f"BLOCK_{old_block_num}", f"BLOCK_{new_block_num}")
                        break

    # Create reformatted_flattened structure with new block IDs
