...
def extract_translatable_html(input_path, lang_code):
    ...
    # === Reorder block IDs by DOM position ===
    ...
    for block_id, block_data in structured_output.items():
        block_type = block_data.get("tag") or block_data.get("attr") or block_data.get("meta")
        key = block_data.get("tag") or block_data.get("attr") or block_data.get("meta") or block_data.get("jsonld")
        if not key:
            continue
        for element in soup.find_all():
            if block_type in TRANSLATABLE_TAGS or element.get(block_type) == key or element.name == key:
