...
def main():
    ...
    # Step 1: Deduplicate
    dedup_input_file = "deduplicated_for_translation.json"
    id_map_file = "id_remap.json"
    deduplicate_segments(
        input_flat_json=args.input,
        dedup_output_json=dedup_input_file,
        id_map_output_json=id_map_file
    )

    # Step 2: Translate only unique segments
    dedup_translations = translate_json_file(
        input_file=dedup_input_file,
        output_file="dedup_translated.json",
    ...
    # Step 3: Remap back to all IDs
    translations = remap_translations(dedup_translations, id_map_file)
